...
export function getItemRelativeTop({
  ...
  const locatedItemTop = scrollPtq * clientHeight;
  const locatedItemOffset = itemOffsetPtq * locatedItemHeight;
 ...
 */
function getLocationItem(scrollPtq, total) {
  const itemIndex = Math.floor(scrollPtq * total);
  const itemTopPtg = itemIndex / total;
  const itemBottomPtg = (itemIndex + 1) / total;
  const itemOffsetPtg = (scrollPtq - itemTopPtg) / (itemBottomPtg - itemTopPtg);
