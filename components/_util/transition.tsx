...
export const getTransitionProps = (transitionName: string, opt: TransitionProps = {}) => {
  const transitionProps: TransitionProps = transitionName
    ? {
        ...
        appear: true,
        // type: 'animation',
        // appearFormClass: `${transitionName}-appear ${transitionName}-appear-prepare`,
        // appearActiveClass: `antdv-base-transition`,
...
// ====================== Collapse Motion ======================
const getCollapseHeight: MotionEventHandler = () => ({ height: 0, opacity: 0 });
...
const collapseMotion = (
  ...
  return {
    ...
    onBeforeEnter: node => {
      ...
      style.value = getCollapseHeight(node);
    },
    onEnter: node => {
      nextTick(() => {
...
const getTransitionName = (rootPrefixClas: string, motion: string, transitionName?: string) => {
  if (transitionName !== undefined) {
    return transitionName;
  }
  return `${rootPrefixClas}-${motion}`;
};
