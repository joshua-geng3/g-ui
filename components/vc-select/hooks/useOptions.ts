...
export default function useOptions<OptionType>(
  ...
  const tmpMergedOptions = shallowRef([]);
  watch(
    [options, children],
    () => {
      if (options.value) {
        tmpMergedOptions.value = toRaw(options.value).slice();
      } else {
        tmpMergedOptions.value = convertChildrenToData(children.value);
      }
    },
    { immediate: true, deep: true },
  );
  watchEffect(() => {
    const newOptions = tmpMergedOptions.value;
