...
const Notification = defineComponent({
  ...
  setup(props, { attrs, expose, slots }) {
    ...
    const add = (originNotice: NoticeContent, holderCallback?: HolderReadyCallback) => {
      ...
      const updateNotices = notices.value.concat();
      if (noticeIndex !== -1) {
        updateNotices.splice(noticeIndex, 1, { notice, holderCallback } as any);
      } else {
        if (maxCount && notices.value.length >= maxCount) {
          notice.key = updateNotices[0].notice.key as Key;
        ...
        updateNotices.push({ notice, holderCallback } as any);
      }
      notices.value = updateNotices;
    };
